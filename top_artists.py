...
def get_top_bot_5(all_artist_data, amount_done):
    ...
    for each_number in range(0, 5):
        ...
        top5_artists.append(
            sorted_all_artist_data[amount_done - (each_number + 1)])  # need to check this works proparly
...
def output_data(average_popularity, top_5_artists, bot_5_artists):
    ...
    edge_responses = [[90, "You need to get of the top 40 playlist , there's a world of music out there!"],
                      [80, "You didn't really know them before they were popular did you? "],
                      [70, "A bit basic but respectable, you have the edge-o-meters approval"],
                      [60, "Very impressive, you're music is far of the beaten track"],
                      [50, "Wow someone's hip. Do you even know what capital is?"],
                      [40, "Your music taste is so far underground it's in china"],
                      [30, "Really? You are clearly the definition of hipster, please teach me your ways"],
                      [20, "Maybe there's a reason the music you listen to is so unpopular?"],
    ...
    for each_in_list in range(0, 5):
        print(str(each_in_list + 1) + ". " + str(top_5_artists[each_in_list][0]) + " Popularity : " + str(
            top_5_artists[each_in_list][1]))
        time.sleep(0.5)
    ...
    for each_in_list in range(0, 5):
        print(str(each_in_list + 1) + ". " + str(bot_5_artists[each_in_list][0]) + " Popularity  : " + str(
            bot_5_artists[each_in_list][1]))
        time.sleep(0.5)
